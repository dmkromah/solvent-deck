...
/* ===== Solvent Delete: Lightweight Post‑Render Attachment (Weekly Plan aware) ===== */
(function () {
  'use strict';

  // 1) Update these selectors to match your Weekly Plan DOM if needed
  const WEEKLY_TASK_ROW_SELECTORS = [
    '[data-task-id]',     // preferred
    '.weekly-task',       // common guess
    '.plan-task',
    '.task-row',
    '.task',
    'li'                  // last resort
  ...
  const CARD_WRAPPER_SELECTORS = [
    '[data-card-id]',
    '.deck-card',
    '.card'
  ];

  // 2) Attach a delete button to weekly tasks
  function attachTaskButtons(root = document) {
    const nodes = root.querySelectorAll(WEEKLY_TASK_ROW_SELECTORS.join(','));
    nodes.forEach((row, i) => {
      // Skip if a delete button already exists
      if (row.querySelector('.btn-delete-task')) return;

      // Ensure we have some id to use
      ...
      if (!taskId) {
        ...
        taskId =
          ...
          row.id ||
          `weekly-${i}`;  // fabricate a stable-ish id for now
      ...
      // Append at the end; tweak if your layout needs a different spot
      row.appendChild(btn);
    });
  }

  // 3) Attach a delete button to cards (already working for you — kept for completeness)
  function attachCardButtons(root = document) {
    const cards = root.querySelectorAll(CARD_WRAPPER_SELECTORS.join(','));
    cards.forEach((card, i) => {
      ...
      if (!cardId) {
        cardId = card.getAttribute('data-id') || card.id || `card-${i}`;
  ...
  function init() {
    attachAll();
    wrapIfExists('renderPlan');   // weekly plan likely re-renders here
